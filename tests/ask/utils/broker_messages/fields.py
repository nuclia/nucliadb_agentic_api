...
class FieldBuilder:
    def __init__(
        self, kbid: str, rid: str, field: str, field_type: rpb.FieldType.ValueType
    ):
        self._kbid = kbid
        self._rid = rid
    ...
    def _extracted_metadata(self) -> rpb.FieldComputedMetadataWrapper:
        if self.__extracted_metadata is None:
            ...
            self.__extracted_metadata.metadata.metadata.last_processing_start.FromDatetime(
                now
            )
    ...
    def _extracted_vectors(self, vectorset: str) -> rpb.ExtractedVectorsWrapper:
        ...
        return self.__extracted_vectors.setdefault(
            vectorset,
            rpb.ExtractedVectorsWrapper(field=self._field_id, vectorset_id=vectorset),
        )
    ...
    def with_extracted_vectors(
        ...
        if split is None:
            ...
            self._extracted_vectors(vectorset).vectors.split_vectors[
                split
    ...
    def with_extracted_paragraph_metadata(
        ...
        if split is None:
            ...
            self._extracted_metadata.metadata.split_metadata[split].paragraphs.append(
                paragraph
            )
            return self._extracted_metadata.metadata.split_metadata[split].paragraphs[
                -1
            ]
    ...
    def with_extracted_entity(
        ...
        *,
        ...
        self._extracted_metadata.metadata.metadata.entities[
            data_augmentation_task_id
        ].entities.append(rpb.FieldEntity(text=name, label=klass, positions=positions))
    ...
    def add_paragraph(
        ...
        if user_labels is not None:
            for labelset, labels_ in user_labels:
                self.with_user_paragraph_labels(
                    paragraph_id.full(), labelset, labels_, split
                )
    ...
    def iter_paragraphs(
        ...
        if split is None:
            ...
            paragraphs = self._extracted_metadata.metadata.split_metadata[
                split
            ].paragraphs
    ...
    async def add_page_preview(self, page: int, content: bytes):
        assert self._field_id.field_type == rpb.FieldType.FILE, (
            "only file fields have page previews"
        )
        ...
        sf = storage.file_extracted(
            ...
            f"file_pages_previews/{page}",
        )
        await storage.upload_object(sf.bucket, sf.key, content)
