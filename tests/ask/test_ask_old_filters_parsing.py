...
async def test_old_filters_parsing(
    ...
    parser = FindParser("kbid", item, fetcher)
    parser._query = retrieval_models.RawQuery()

    with patch(
        "hyperforge_nucliadb_agentic.ask.search.parsers.fetcher.rpc.labelsets",
...
async def test_old_filters_parsing_invalid_combinations(
    ...
    with (
        ...
        await parser._parse_filters(reader_sdk=None)  # type: ignore
