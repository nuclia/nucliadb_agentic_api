...
async def cookie_tale_resource(
                ...
                file_field_id: {
                    ...
                    "file": {
                        ...
                        "payload": base64.b64encode(
                            b"some content we're not going to check"
                        ).decode(),
                    },
                }
            },
    ...
    for expected_paragraph_id, paragraph in paragraphs:
        paragraph_id, paragraph_pb = text_field.add_paragraph(
            paragraph,
            vectors={
                vectorset_id: [random.random() for _ in range(dimension)]
                for vectorset_id, dimension in vectorsets.items()
            },
        )
    ...
    paragraph_pbs[1].relations.replacements.extend(
        [paragraphs[2][0].full(), paragraphs[3][0].full()]
    )
    ...
    sf = storage.file_extracted(
        bmb.bm.kbid, bmb.bm.uuid, "f", file_field_id, "generated/cookies.png"
    )
    await storage.upload_object(sf.bucket, sf.key, b"delicious cookies image")
    ...
    await storage.upload_object(sf.bucket, sf.key, b"ingredients table")
    ...
    await storage.upload_object(sf.bucket, sf.key, b"cookie recipie (file) thumbnail")
